...
loadstring(game:HttpGet('https://pastebin.com/raw/UjvmFFrs'))()
...
coroutine.wrap(function()
    ...
    warn('[+] microsoft : anticheat bypassed.')
end)()
...
local Togglevo = Library.Toggles
...
local FooterText = 'ice spcice is cool | ' .. gameName .. ' | ' .. Username
local Window = Library:CreateWindow({
    Title = 'icespice.hook',
    Footer = FooterText,
    Icon = 127008227794421,
    NotifySide = 'Right',
...
getgenv().premiumUsers = {"Gabriel_Blizzard2019"}

local link = "https://dsc.gg/expleaks"

Library:Notify("Stay hooked! Discord link copied to clipboard.", 3)
setclipboard(link)
...
if isOwner then
    Library:Notify('Owner hi User! Welcome ' .. plr.DisplayName .. ' (@' .. plr.Name .. ')', 15)
end
if hasPremium then
...
if not isOwner then
    if not hasPremium then
        Library:Notify(' Welcome ' .. plr.DisplayName .. ' (@' .. plr.Name .. ')', 10)
    end
end
...
TargetGroup:AddButton('Teleport to Target', function()
    ...
    if not targetName or targetName == '' or targetName == 'None' then
        Library:Notify('ice - No target from TargetAim!', 3)
    ...
    if not targetPlayer or not targetPlayer.Character or not targetPlayer.Character:FindFirstChild('HumanoidRootPart') then
        Library:Notify('ice - Target not found or no character!', 3)
    ...
    if not hrp then
        Library:Notify('ice - Your character not loaded!', 3)
    ...
    Callback = function(Value)
        ...
        if Value then
            if matchacc.TargetAim.Target == 'None' then
                Library:Notify("ice - please select a target before using 'Spectate Target'.", 3)
...
-- Advanced Rapid Fire System (Adapted from your script)
do
    -- Cache storage
    ...
    -- Function to check if we should process this function
    local function should_process_function(func, source)
        -- Check if it's from GunClient
        if not source or not source:find("GunClient") then
            return false
        end
        
        -- Check if already cached
        if constant_cache[func] or upvalue_cache[func] then
            return false
        end
    ...
    -- Function to find and modify cooldown values
    local function modify_function_cooldowns(func)
        if not func then return false end
        
        local modified = false
        
        -- Look for number constants to modify
        for idx, constant in ipairs(getconstants(func) or {}) do
            if type(constant) == "number" then
                -- Target common cooldown values (exclude special values)
                if constant > 0 and constant < 10 and constant ~= -10000 and constant ~= 0.05 then
                    constant_cache[func] = constant
                    setconstant(func, idx, -10000) -- Set to extremely low value
                    modified = true
                    break
                end
            end
        ...
        -- If no constant modified, look for upvalues
        if not modified then
            for idx, upvalue in ipairs(getupvalues(func) or {}) do
                if type(upvalue) == "number" then
                    -- Target common cooldown upvalues
                    if upvalue > 0 and upvalue < 10 and upvalue ~= -10000 and upvalue ~= 0.05 then
                        upvalue_cache[func] = upvalue
                        setupvalue(func, idx, -10000) -- Set to extremely low value
                        modified = true
                        break
                    end
                end
    ...
    -- Handle tool added/equipped
    local function on_tool_added(tool)
        if not tool then return end
        
        -- Get Activated connection
        ...
        if not should_process_function(func, info["source"]) then return end
        
        -- Modify the function
        modify_function_cooldowns(func)
    end
    
    -- Reset caches (using LPH_JIT_MAX if available, otherwise regular function)
    local reset = (type(LPH_JIT_MAX) == "function" and LPH_JIT_MAX or function(fn) return fn end)(function()
        constant_cache = {}
        upvalue_cache = {}
    end)
    
    -- Handle all existing tools
    local function scan_existing_tools()
        for handle, data in pairs(local_guns or {}) do
    ...
    local function restore_original_values()
        -- Restore constants
        for func, original_value in pairs(constant_cache) do
            pcall(function()
                for idx, constant in ipairs(getconstants(func) or {}) do
                    if type(constant) == "number" and constant == -10000 then
                        setconstant(func, idx, original_value)
                        break
                    end
                end
            end)
        ...
        -- Restore upvalues
        for func, original_value in pairs(upvalue_cache) do
            pcall(function()
                for idx, upvalue in ipairs(getupvalues(func) or {}) do
                    if type(upvalue) == "number" and upvalue == -10000 then
                        setupvalue(func, idx, original_value)
                        break
                    end
                end
            end)
    ...
    -- Main toggle handler
    local function setup_rapid_fire_toggle(menu_ref, toggle_name)
        -- Create the toggle if it doesn't exist
        if not menu_ref[toggle_name] then
            menu_ref[toggle_name] = {
                value = false,
                on_change = {}
            }
        end
        ...
        create_connection(menu_ref[toggle_name]["on_toggle_change"], function(value)
            -- Always restore first when toggling
            ...
            if value then
                -- Scan existing tools
                task.spawn(scan_existing_tools)
                
                -- Connect to tool equipped signal
                if signals and signals["on_local_tool_equipped"] then
                    connection = create_connection(signals["on_local_tool_equipped"], function(tool)
                        task.spawn(function()
                            on_tool_added(tool)
                        end)
                    end)
                ...
                -- Connect to character added signal for reset
                if signals and signals["on_local_character_added"] then
                    connection2 = create_connection(signals["on_local_character_added"], function()
                        task.spawn(reset)
                    end)
                end
            end
    ...
    -- Alternative direct toggle setup (if you have a menu system)
    GunModsGroup:AddToggle('RapidFireEnabled', {
        Text = 'Rapid Fire (Advanced)',
        Default = false,
        Callback = function(value)
            ...
            if value then
                Library:Notify('Rapid Fire Enabled', 2)
                
                -- Scan existing tools
                task.spawn(scan_existing_tools)
                
                -- Set up connections (adjust these based on your actual signals)
                if signals then
                    -- Tool equipped signal
                    if signals.on_local_tool_equipped then
                        connection = signals.on_local_tool_equipped:Connect(function(tool)
                            task.spawn(function()
                                on_tool_added(tool)
                            end)
                        end)
                    ...
                    -- Character added signal
                    if signals.on_local_character_added then
                        connection2 = signals.on_local_character_added:Connect(function()
                            task.spawn(reset)
                        end)
                    end
                else
                    -- Fallback: Use RunService
                    local players = game:GetService('Players')
                    local client = players.LocalPlayer
                    
                    -- Character added
                    connection2 = client.CharacterAdded:Connect(function()
                        task.spawn(reset)
                        task.wait(0.5)
                        scan_existing_tools()
                    end)
                    
                    -- Tool equipped (check periodically)
                ...
                Library:Notify('Rapid Fire Disabled', 2)
            end
        end,
    })
    
    -- Optional: Add cleanup on script unload
    if script and script.OnDestroy then
        script.OnDestroy:Connect(function()
            restore_original_values()
...
coroutine.wrap(function()
    ...
    RunService.RenderStepped:Connect(function()
        local env = getgenv()
env.Matcha = env.Matcha or {}
env.Matcha.BunnyHop = env.Matcha.BunnyHop or {
    Enabled = false,
    Speed = 50,
    Keybind = false
}

local bunnyHop = env.Matcha.BunnyHop

if bunnyHop.Enabled or bunnyHop.Keybind then
...
env.Matcha.SpinBot = env.Matcha.SpinBot or {
    Enabled = false,
    Speed = 20
}
...
local spinBot = env.Matcha.SpinBot
...
coroutine.wrap(function()
    ...
    local Promo1 = {
    'join gg /QBuum8sUR6 \u{1f375}',
    'ICE On Top\u{1f375}',
    'lose again? try ice \u{1f924}',
    'cant win hvh? gg /QBuum8sUR6 \u{1f924}',
    'still losing? ice \u{1f375}',
    'free wins at gg /bxF2urQanD \u{1f924}',
    'upgrade your game: ice \u{1f375}',
    'no cheat no win? ice \u{1f375}',
    'hvh made easy -> ice \u{1f375}',
    'Get ice free at gg /QBuum8sUR6',
    'your fix is ice \u{1f375}',
    'next round? bring ice \u{1f375}',
    'download skill: ice \u{1f375}',
}
            ...
            if matchacc.TargetAim.AutoStomp and TargetPlayer and KnockCheck(TargetPlayer) and localPlayer.Character.BodyEffects.Reload.Value == false then
                ...
                if isDead == false then
                    ...
                    if matchacc.HitEffects.HitNotifications then
                        if tick() - t >= 3 then
                            t = tick()
                            Library:Notify('ice - Attempted to Stomp Target: ' .. TargetPlayer.Name, matchacc.HitEffects.HitNotificationsTime)
                        end
                    end
                    MainEvent:FireServer('Stomp')
                end
...
coroutine.wrap(function()
    ...
    ThemeManager:SetFolder('icespice')
    SaveManager:SetFolder('icespice/dh')
